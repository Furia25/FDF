...
/*+#+#+#+#+#+   +#+   */
/*   Created: 2025/01/14 03:35:16 by val   #+##+# */
/*   Updated: 2025/01/14 03:35:22 by val  ###   ########.fr   */
/**/
/* ************************************************************************** */
...
t_quaternion	quaternion_normalize(t_quaternion q)
{
	...
	length = fast_sqrt(q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w);
	if (length > 0.0f)
	{
...
t_quaternion	quaternion_from_axis_angle(t_vect3 axis, float angle)
{
	...
	half_angle = (angle * M_PI / 180.0f) * 0.5f;
	s = sin(half_angle);
